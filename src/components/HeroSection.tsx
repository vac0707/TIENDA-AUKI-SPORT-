...
import { ShoppingBag, Phone, ArrowRight, ShieldCheck, Sparkles, Zap, Tag } from 'lucide-react';
import { SiteConfig, PromotionsConfig } from '../types';
...
export const HeroSection: React.FC<HeroSectionProps> = ({ 
  ...
  const badgeText = promotionsConfig?.badgeText || "Especial Fiestas Patrias 🇵🇪 2026";
  const headline = promotionsConfig?.headline || "¡CELEBRA AL PERÚ CON LO MEJOR!";
  const subheadline = promotionsConfig?.subheadline || "Catálogo oficial AUKI SPORT. Zapatillas 100% Originales Nike, Adidas, New Balance, Reebok, Asics, CAT y Merrell con descuentos exclusivos.";
  const discounts = promotionsConfig?.discounts && promotionsConfig.discounts.length >= 3 
    ? promotionsConfig.discounts 
    : ["20%", "30%", "50%"];

  const bannerImg = siteConfig?.mainBannerUrl || promotionsConfig?.bannerImageUrl || "https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&q=80&w=800";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-black italic uppercase tracking-tighter leading-[0.9]">
              {headline}
            </h1>
            ...
            <div className="grid grid-cols-3 gap-3 max-w-lg pt-2">
              <div 
                onClick={() => onSelectCategory(promotionsConfig?.activeCategory || 'Fiestas Patrias')}
              ...
              >
                <span className="text-2xl font-black text-red-500 group-hover:scale-110 block transition-transform">{discounts[0]}</span>
              ...
              <div 
                ...
                className="bg-red-950/40 border border-red-500/30 p-3 rounded-xl cursor-pointer hover:border-red-500 hover:bg-red-900/40 transition-all text-center group"
              >
                <span className="text-2xl font-black text-red-500 group-hover:scale-110 block transition-transform">{discounts[1]}</span>
                <span className="text-[10px] font-bold text-white/60 uppercase tracking-tight block">DESCUENTO</span>
              </div>
              <div 
              ...
              >
                <span className="text-2xl font-black text-white group-hover:scale-110 block transition-transform">{discounts[2]}</span>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-4">
              <button 
                ...
                className="px-8 py-4 bg-red-600 hover:bg-red-500 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all flex items-center gap-3 shadow-xl shadow-red-600/30 hover:scale-105 active:scale-95 cursor-pointer"
              >
              ...
              <a 
                href={`https://wa.me/${whatsappNum}?text=Hola%20AUKI%20SPORT,%20deseo%20hacer%20un%20pedido%20del%20cat%C3%A1logo`}
                target="_blank"
                rel="noreferrer"
            ...
            <div className="pt-4 flex items-center gap-6 text-xs text-white/50 border-t border-white/10">
              <div className="flex items-center gap-2">
                ...
                <span>Pedidos directos: <strong className="text-white">{phoneDisplay}</strong></span>
              </div>
            </div>
          </motion.div>
            ...
            <div className="relative rounded-3xl overflow-hidden border border-red-500/30 bg-gradient-to-b from-neutral-900 to-black p-6 shadow-2xl shadow-red-950/50">
              ...
              <div className="relative aspect-[4/3] rounded-2xl overflow-hidden my-4 group">
                ...
                <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between">
                  ...
                  <button 
                    onClick={onExploreCatalog}
                    className="p-3 bg-red-600 text-white rounded-full hover:bg-white hover:text-red-600 transition-colors shadow-lg cursor-pointer"
                  >
                    <ShoppingBag size={18} />
                  </button>
                </div>
